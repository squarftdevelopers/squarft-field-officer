...
export default function ProjectLeadFormSheet({ visible, translateY, screenHeight, onClose }) {
    ...
    const sendContactOtp = async (kind, phone) => {
        ...
        try {
            ...
            const result = await leadsAPI.sendContactOtp(phone);
            setOtpState((s) => ({ ...s, [kind]: { otpToken: result.otp_token, code: "", sent: true, verifiedToken: null, loading: false } }));
        } catch (error) {
    ...
    const verifyContactOtp = async (kind) => {
        const state = otpState[kind];
        try {
            setOtpState((s) => ({ ...s, [kind]: { ...s[kind], loading: true } }));
    ...
    const handleSelectDeveloper = (user) => {
        setForm((prev) => ({
            ...
            existingDeveloperId: user.id,
        }));
        setOtpState((s) => ({ ...s, builder: { selectedExisting: true, verifiedToken: "existing" } }));
    ...
    const handleSave = async () => {
        ...
        const payload = {
            ...
            existing_developer_id: form.existingDeveloperId,
            builder_verified_token: form.existingDeveloperId ? undefined : otpState.builder.verifiedToken,
            responsible_person_name: form.responsiblePerson.trim(),
            responsible_person_contact: form.responsibleMobile.trim(),
            responsible_verified_token: otpState.responsible.verifiedToken,
        ...
        try {
            ...
            dispatch(
                addProject({
                    ...
                    responsiblePhoneNumber: form.responsibleMobile.trim(),
                    city: form.city.trim(),
                ...
                <View className="-mt-5 flex-1 overflow-hidden rounded-t-[20px] bg-white">
                    ...
                    {currentStep === 0 && (
                        <View className="gap-6">
                            ...
                            <View className="mb-4">
                                <Text className="mb-1.5 text-xs font-lato-bold text-black">Contact Person Mobile *</Text>
                                <View className="h-12 flex-row items-center rounded-xl border border-gray-200 bg-white px-4">
                                    ...
                                    <TextInput
                                        ...
                                        className="flex-1 text-[13px] font-lato text-gray-800"
                                        maxLength={10}
                                        style={{ paddingVertical: 0, textAlignVertical: "center", includeFontPadding: false }}
                                    />
                                </View>
                            ...
                            {!form.existingDeveloperId && (
                                <View className="-mt-4 mb-4 flex-row items-center" style={{ columnGap: 8 }}>
                                    {otpState.builder.sent && !otpState.builder.verifiedToken && (
                                        <TextInput value={otpState.builder.code || ""} onChangeText={(code) => setOtpState((s) => ({ ...s, builder: { ...s.builder, code } }))} placeholder="6-digit OTP" keyboardType="number-pad" maxLength={6} className="h-11 flex-1 rounded-xl border border-gray-200 bg-white px-3 text-[13px]" />
                                    )}
                                    <TouchableOpacity onPress={() => otpState.builder.sent && !otpState.builder.verifiedToken ? verifyContactOtp("builder") : sendContactOtp("builder", form.mobile)} disabled={otpState.builder.loading || Boolean(otpState.builder.verifiedToken)} className={`h-11 items-center justify-center rounded-xl px-4 ${otpState.builder.verifiedToken ? "bg-[#DCFCE7]" : "bg-[#4A43EC]"}`}>
                                        {otpState.builder.loading ? (
                                            <ActivityIndicator size="small" color="#FFFFFF" />
                                        ) : (
                                            <Text className={`text-xs font-lato-bold ${otpState.builder.verifiedToken ? "text-[#15803D]" : "text-white"}`}>{otpState.builder.verifiedToken ? "Verified" : otpState.builder.sent ? "Verify OTP" : "Send OTP"}</Text>
                                        )}
                                    </TouchableOpacity>
                                </View>
                            )}

                            <View className="mb-4 rounded-xl border border-[#DDE2FF] bg-[#F8F9FF] p-3">
                                <Text className="mb-3 text-xs font-lato-bold text-[#312E81]">Responsible Person *</Text>
                                <TextInput
                                ...
                                <View className="mt-2 flex-row items-center" style={{ columnGap: 8 }}>
                                    {otpState.responsible.sent && !otpState.responsible.verifiedToken && (
                                        <TextInput
                                            value={otpState.responsible.code || ""}
                                            onChangeText={(code) => setOtpState((s) => ({ ...s, responsible: { ...s.responsible, code } }))}
                                            placeholder="6-digit OTP"
                                            keyboardType="number-pad"
                                            maxLength={6}
                                            className="h-11 flex-1 rounded-xl border border-gray-200 bg-white px-3 text-[13px]"
                                        />
                                    )}
                                    <TouchableOpacity
                                        onPress={() => otpState.responsible.sent && !otpState.responsible.verifiedToken ? verifyContactOtp("responsible") : sendContactOtp("responsible", form.responsibleMobile)}
                                        disabled={otpState.responsible.loading || Boolean(otpState.responsible.verifiedToken)}
                                        className={`h-11 items-center justify-center rounded-xl px-4 ${otpState.responsible.verifiedToken ? "bg-[#DCFCE7]" : "bg-[#4A43EC]"}`}
                                    >
                                        {otpState.responsible.loading ? (
                                            <ActivityIndicator size="small" color="#FFFFFF" />
                                        ) : (
                                            <Text className={`text-xs font-lato-bold ${otpState.responsible.verifiedToken ? "text-[#15803D]" : "text-white"}`}>
                                                {otpState.responsible.verifiedToken ? "Verified" : otpState.responsible.sent ? "Verify OTP" : "Send OTP"}
                                            </Text>
                                        )}
                                    </TouchableOpacity>
                                </View>
                    ...
                    {currentStep === 1 && (
                        <View className="gap-6">
                            ...
                            >
                                <Ionicons name="map-outline" size={20} color="#4A43EC" />
                            ...
                            {pickedCoordinate ? (
                                <View className="rounded-2xl border border-[#DDE2FF] bg-[#F8F9FF] px-4 py-3">
                                    <View className="flex-row items-center">
                                        <Ionicons name="location" size={20} color="#4A43EC" />
                                        <View className="ml-3 flex-1">
                                            <Text className="text-[12px] font-lato-bold text-[#312E81]">Location selected</Text>
                                            <Text className="mt-1 text-[11px] leading-4 text-gray-500" numberOfLines={2}>{form.fullAddress}</Text>
                                        </View>
                                        <TouchableOpacity onPress={() => setMapPickerVisible(true)} className="min-h-11 justify-center px-2">
                                            <Text className="text-[12px] font-lato-bold text-[#4A43EC]">Change</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                            ) : null}
                            ...
                            <LocationMapPicker
                                ...
                                initialAddress={{ location: form.fullAddress, city: form.city, state: form.state, pincode: form.pincode }}
